...
function fnSave() {
	...
	if (!fnValidateRequired(num,"<spring:message code="dev.err.com.requiredSearchNum"/>")) return false;

	if(document.detailForm.mode.value == "insert") {
		document.detailForm.action = "<c:url value='/pattern/p11/insertEmployee.do'/>";
	}else{
		document.detailForm.action = "<c:url value='/pattern/p11/updateEmployee.do'/>";
	}
	document.detailForm.submit();
}
function fnRetrieve() {
	document.detailForm.action = "<c:url value='/pattern/p11/retrieveEmployee.do'/>";
	document.detailForm.submit();
}
...
<div id="LblockBody">
	...
	<div id="LblockBodyMain">
		<div id="LblockDetail01" class="LblockDetail">
			<form:form modelAttribute="result" name="detailForm" method="post" onsubmit="fnRetrieve();return false;">
				...
				<input type="hidden" name="searchNum" value="${param.searchNum}" />
	...
	<div id="LblockButton">
		<a class="Lbtn" href="#" onclick="fnRetrieve();"><span><spring:message code="common.label.cancel" /></span></a>
